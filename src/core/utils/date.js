...
export default {
    ...
    let returnValue =
      date.getMonth() + 1 >= 10
        ...
        : '0' +
          ...
          date.getFullYear();
    if (format === true) {
      returnValue += '  ' + dateUtils.getFullTime(date);
    }
    return returnValue;
  },
  ...
  dateParse(field, e, time, data) {
    ...
    if (field) {
      ...
      if (time) {
        var serverTimeZoneOffSet = secureStorage.getItem('serverTimeZone') || 0;
        var timeZoneOffSet = secureStorage.getItem('timeZoneOffSet') || 0;
  ...
  DateTimeWithTimeZone: function (value) {
    ...
    var date = val instanceof Date ? val : dateUtils.parse(val);
    var localDate = date ? dateUtils.getDateTime(date) : '';
    date =
  ...
  DateWithLocalTimeZone: function (value) {
    var val = value.value;
    if (!val) {
      val = value;
    }
    ...
    var date = val instanceof Date ? val : dateUtils.parse(val);
    var localDate = date ? dateUtils.getLocalDateTime(date) : '';
    date =
      localDate && localDate.getFullYear() > 1899
        ? moment(localDate).format(Util.dateFormat)
        : '';
    var timeZone = localDate
      ? localDate
          .toString()
  ...
  DateLocalizer: function (value) {
    ...
    var date = val instanceof Date ? val : dateUtils.parse(val);
    var localDate = dateUtils.getLocalDateTime(date);
    date =
      localDate && localDate.getFullYear() > 1899
        ? moment(localDate).format(Util.dateTimeFormat)
        : '';
